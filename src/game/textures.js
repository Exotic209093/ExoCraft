...
// ---------------------------------------------------------------------------
// Wave P1 — procedural pixel-art ITEM icons.
// Every icon is an original 16x16 grid-string painting: rows of 16 chars where
// '.' = transparent and letters map to palette roles. One SHAPE is shared by a
// whole item family (all pickaxes use the same grid) and a per-item PALETTE
// tints it, so 60+ items cost ~30 grids. Painted once, cached, and consumed by
// the hotbar, inventory/container UIs, ground item entities and the viewmodel.
//   h/H = handle dark/light (shared wood colors unless the palette overrides)
//   a/b/c = material dark/mid/light   w,g,r,s,f = per-grid accents (white,
//   green, red, string/silver, fill) — palettes may override any of them.
// ---------------------------------------------------------------------------

const ICON_HANDLE_DARK = "#5a3a1c";
const ICON_HANDLE_LIGHT = "#8a5c30";
const ICON_EXTRA_DEFAULT = { w: "#f4f4f4", g: "#4a9040", r: "#c02020", s: "#cfd8dc" };
...
// Material palettes: a = dark, b = mid, c = light. Optional accent overrides.
const ICON_PALS = {
  wood:       { a: "#6b4a26", b: "#9a6a3b", c: "#c08a52" },
  stone:      { a: "#5a5f66", b: "#80878f", c: "#a8afb8" },
  copper:     { a: "#8a4f22", b: "#c47a3a", c: "#e8a05c" },
  iron:       { a: "#8a8f96", b: "#c8cdd4", c: "#eef2f6" },
  gold:       { a: "#9a7a10", b: "#e8c840", c: "#f8e88a" },
  diamond:    { a: "#1e9a8e", b: "#50e8d8", c: "#b2f8f0" },
  bone:       { a: "#9a927a", b: "#d8d0b8", c: "#f0ecdc" },
  reinforced: { a: "#3a3f48", b: "#5a626e", c: "#8a94a2" },
  vanguard:   { a: "#7a1626", b: "#b22c42", c: "#e05a6e" },
  delver:     { a: "#1e3a7a", b: "#3a62b2", c: "#6a92e0" },
  leather:    { a: "#5c3a1e", b: "#8b5a2b", c: "#b07a42" },
  apple:      { a: "#7a1010", b: "#c02020", c: "#e04848" },
  appleCooked:{ a: "#5c2a10", b: "#a0522d", c: "#c8763e", g: "#7a5a20" },
  bread:      { a: "#7a5218", b: "#b8842e", c: "#e0aa50" },
  wheatPal:   { c: "#e0c460", b: "#c8a040", h: "#a08030" },
  seedsPal:   { g: "#3a7a2a", b: "#5aa03a" },
  beefRaw:    { a: "#7a1a1a", b: "#c03030", c: "#d85050", w: "#f0d8d0" },
  steak:      { a: "#4a2210", b: "#7a4224", c: "#9a5c36", w: "#d8c0a8" },
  porkRaw:    { a: "#b05868", b: "#e08a98", c: "#f0b0ba", w: "#f8e0e4" },
  porkCooked: { a: "#8a5a2e", b: "#c08a4e", c: "#d8a86a", w: "#ecd8b8" },
  chickenRaw: { b: "#e8b8a8", c: "#f4d4c8", w: "#f4f0e4" },
  chickenCooked:{ b: "#b87838", c: "#d89a54", w: "#f0e4d0" },
  coal:       { a: "#141418", b: "#2e2e34", c: "#4a4a52" },
  charcoal:   { a: "#1e1410", b: "#3a2a20", c: "#544036" },
  flint:      { a: "#26262c", b: "#40404a", c: "#5c5c68" },
  redstoneDust:{ a: "#7a0e0a", b: "#c02020", c: "#e84838" },
  refined:    { a: "#6a7078", b: "#98a0a8", c: "#c4ccd4" },
  whiteFluff: { b: "#c8c8cc", c: "#e8e8ea", w: "#f8f8fa" },
  featherPal: { w: "#f0f0ee", s: "#b8b8c0", h: "#c8a040" },
  stringPal:  { s: "#e4e4e0" },
  hidePal:    { b: "#8a5c30", c: "#b8834a" },
  bucketEmpty:{ a: "#5a5f66", b: "#80878f", c: "#a8afb8", f: "#3c4046" },
  bucketWater:{ a: "#5a5f66", b: "#80878f", c: "#a8afb8", f: "#2b6ccc" },
  bucketLava: { a: "#5a5f66", b: "#80878f", c: "#a8afb8", f: "#ff6600" },
  shearsPal:  { b: "#98a0a8", c: "#d0d6dc", a: "#5a5f66", r: "#8a2a2a" },
  bowPal:     { s: "#e4e4e0", b: "#8a5c30" },
...
// few overrides where the painted icon reads better than the block tile).
const ICON_SPECS = {
  // Tools per tier
  wood_pickaxe:  { grid: "pickaxe", pal: "wood" },
  stone_pickaxe: { grid: "pickaxe", pal: "stone" },
  copper_pickaxe:{ grid: "pickaxe", pal: "copper" },
  iron_pickaxe:  { grid: "pickaxe", pal: "iron" },
  diamond_pickaxe:{ grid: "pickaxe", pal: "diamond" },
  reinforced_pickaxe: { grid: "pickaxe", pal: "reinforced" },
  deep_delver_pickaxe:{ grid: "pickaxe", pal: "delver" },
  wood_axe:      { grid: "axe", pal: "wood" },
  stone_axe:     { grid: "axe", pal: "stone" },
  iron_axe:      { grid: "axe", pal: "iron" },
  diamond_axe:   { grid: "axe", pal: "diamond" },
  wood_shovel:   { grid: "shovel", pal: "wood" },
  stone_shovel:  { grid: "shovel", pal: "stone" },
  iron_shovel:   { grid: "shovel", pal: "iron" },
  diamond_shovel:{ grid: "shovel", pal: "diamond" },
  wood_hoe:      { grid: "hoe", pal: "wood" },
  stone_hoe:     { grid: "hoe", pal: "stone" },
  iron_hoe:      { grid: "hoe", pal: "iron" },
  // Weapons
  wood_sword:    { grid: "sword", pal: "wood" },
  bone_blade:    { grid: "sword", pal: "bone" },
  copper_blade:  { grid: "sword", pal: "copper" },
  iron_sword:    { grid: "sword", pal: "iron" },
  diamond_sword: { grid: "sword", pal: "diamond" },
  vanguard_blade:{ grid: "sword", pal: "vanguard" },
  bow:           { grid: "bow", pal: "bowPal" },
  arrow:         { grid: "arrow", pal: "arrowPal" },
  shears:        { grid: "shears", pal: "shearsPal" },
  // Buckets
  empty_bucket:  { grid: "bucket", pal: "bucketEmpty" },
  water_bucket:  { grid: "bucket", pal: "bucketWater" },
  lava_bucket:   { grid: "bucket", pal: "bucketLava" },
  // Materials
  stick:         { grid: "stick", pal: "wood" },
  plank:         { grid: "plank", pal: "wood" },
  coal:          { grid: "lump", pal: "coal" },
  charcoal:      { grid: "lump", pal: "charcoal" },
  flint:         { grid: "lump", pal: "flint" },
  refined_stone: { grid: "lump", pal: "refined" },
  iron_ingot:    { grid: "ingot", pal: "iron" },
  gold_ingot:    { grid: "ingot", pal: "gold" },
  copper_ingot:  { grid: "ingot", pal: "copper" },
  diamond:       { grid: "gem", pal: "diamond" },
  redstone:      { grid: "dust", pal: "redstoneDust" },
  bone_shard:    { grid: "shard", pal: "bone" },
  feather:       { grid: "feather", pal: "featherPal" },
  string:        { grid: "string", pal: "stringPal" },
  leather:       { grid: "hide", pal: "hidePal" },
  wool:          { grid: "puff", pal: "whiteFluff" },
  // Food
  apple:         { grid: "apple", pal: "apple" },
  cooked_apple:  { grid: "apple", pal: "appleCooked" },
  bread:         { grid: "bread", pal: "bread" },
  wheat:         { grid: "wheat", pal: "wheatPal" },
  seeds:         { grid: "seeds", pal: "seedsPal" },
  raw_beef:      { grid: "meat", pal: "beefRaw" },
  steak:         { grid: "meat", pal: "steak" },
  raw_porkchop:  { grid: "meat", pal: "porkRaw" },
  cooked_porkchop:{ grid: "meat", pal: "porkCooked" },
  raw_chicken:   { grid: "drumstick", pal: "chickenRaw" },
  cooked_chicken:{ grid: "drumstick", pal: "chickenCooked" },
  // Armor
  leather_helmet:     { grid: "helmet", pal: "leather" },
  leather_chestplate: { grid: "chestplate", pal: "leather" },
  leather_leggings:   { grid: "leggings", pal: "leather" },
  leather_boots:      { grid: "boots", pal: "leather" },
  iron_helmet:        { grid: "helmet", pal: "iron" },
  iron_chestplate:    { grid: "chestplate", pal: "iron" },
  iron_leggings:      { grid: "leggings", pal: "iron" },
  iron_boots:         { grid: "boots", pal: "iron" },
  diamond_helmet:     { grid: "helmet", pal: "diamond" },
  diamond_chestplate: { grid: "chestplate", pal: "diamond" },
  diamond_leggings:   { grid: "leggings", pal: "diamond" },
  diamond_boots:      { grid: "boots", pal: "diamond" },
  // Specials
...
function paintIconGrid(ctx, gridName, palName) {
  const grid = ICON_GRIDS[gridName];
  const pal = ICON_PALS[palName] || {};
  ...
  for (let y = 0; y < 16; y += 1) {
    ...
    for (let x = 0; x < 16; x += 1) {
      ...
      if (!ch || ch === ".") continue;
      let color;
      if (ch === "h") color = pal.h || ICON_HANDLE_DARK;
      else if (ch === "H") color = pal.H || ICON_HANDLE_LIGHT;
      else color = pal[ch] || ICON_EXTRA_DEFAULT[ch];
      if (!color) continue;
      ctx.fillStyle = color;
      ctx.fillRect(x, y, 1, 1);
    }
...
/** True if this itemId has a dedicated painted pixel-art icon. */
export function hasItemIconArt(itemId) {
  return Object.prototype.hasOwnProperty.call(ICON_SPECS, itemId);
}
...
export function getItemIconCanvas(itemId, placeBlockType) {
  ...
  // Wave P1 — painted pixel-art icon takes precedence (e.g. redstone dust shows a
  // dust pile rather than the wire block tile).
  const iconSpec = ICON_SPECS[itemId];
  if (iconSpec) {
    ...
    if (paintIconGrid(srcCtx, iconSpec.grid, iconSpec.pal)) {
      ctx.drawImage(src, 0, 0, 16, 16, 0, 0, ICON_SIZE, ICON_SIZE);
      drawnFromAtlas = true;
    }
  }
